...
class YOLOv1(nn.Module):
    ...
    @staticmethod
    def get_norm_xywh(pred):
        pred_norm_xywh1 = rearrange(
            torch.sigmoid(pred[0: 4, ...]), pattern="n h w -> (h w) n",
        )
        pred_norm_xywh2 = rearrange(
            torch.sigmoid(pred[5: 9, ...]), pattern="n h w -> (h w) n",
        )
        return torch.cat([pred_norm_xywh1, pred_norm_xywh2], dim=0)

    def denormalize_xywh(self, norm_xywh):
        xywh = norm_xywh.clone()
        ori_shape = xywh.shape
        xywh[..., 0: 2] *= self.cell_size
        xywh = xywh.view(self.n_cells, self.n_cells, self.n_bboxes, 4)
        grid_lts = torch.arange(
            0, self.img_size, self.cell_size, device=norm_xywh.device,
        )
        xywh[..., 0] += einops.repeat(
            grid_lts, pattern="a -> a n b", n=self.n_cells, b=self.n_bboxes,
        )
        xywh[..., 1] += einops.repeat(
            grid_lts, pattern="a -> n a b", n=self.n_cells, b=self.n_bboxes,
        )
        xywh = xywh.view(ori_shape)
        xywh[..., 2: 4] *= self.img_size
        return xywh

    def get_ltrb(self, pred):
        pred_norm_xywh = self.get_norm_xywh(pred)
        pred_xywh = self.denormalize_xywh(pred_norm_xywh)
        return self.xywh_to_ltrb(pred_xywh)

    def _get_responsibility_mask(self, pred, gt_ltrb):
        """
        ...
        """
        pred_ltrb = self.get_ltrb(pred)
        iou = get_iou(pred_ltrb, gt_ltrb)
        _, max_iou_idx = torch.max(iou, dim=1, keepdim=False)
        return F.one_hot(max_iou_idx, num_classes=self.n_bboxes).bool()

    def get_object_mask(self, gt_ltrb):
        obj_mask = torch.zeros(
            size=(self.n_cells, self.n_cells), dtype=torch.bool,
        )
        x = (gt_ltrb[..., 0] + gt_ltrb[..., 2]) / 2
        y = (gt_ltrb[..., 1] + gt_ltrb[..., 3]) / 2
        obj_mask[
            (x // self.cell_size).long(), (y // self.cell_size).long()
        ] = True
        return obj_mask
...
if __name__ == "__main__":
    ...
    gt_ltrbs = annots["ltrbs"]
    for batch_idx in range(out.size(0)):
        gt_ltrb = gt_ltrbs[batch_idx]
        pred = out[batch_idx]
        ...
        pred_conf[obj_mask].shape, iou[obj_mask.repeat(1, 2)].shape
